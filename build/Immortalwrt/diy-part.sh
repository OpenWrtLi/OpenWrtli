...
# 后台IP设置
export Ipv4_ipaddr="10.0.0.253"            # 修改openwrt后台地址(填0为关闭)
export Netmask_netm="255.255.255.0"         # IPv4 子网掩码（默认：255.255.255.0）(填0为不作修改)
export Op_name="Jydn"                # 修改主机名称为OpenWrt-123(填0为不作修改)
...
# 默认主题设置
export Mandatory_theme="bootstrap"              # 将argon替换您需要的主题为必选主题(可自行更改您要的,源码要带此主题就行,填写名称也要写对) (填写主题名称,填0为不作修改)
...
# 更换固件内核
export Replace_Kernel="6.1.64"                    # 更换内核版本,在对应源码的[target/linux/架构]查看patches-x.x,看看x.x有啥就有啥内核了(填入内核x.x版本号,填0为不作修改)
